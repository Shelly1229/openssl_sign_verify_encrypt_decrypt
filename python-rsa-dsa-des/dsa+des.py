...
for file_name in message_all:
    ...
    if is_verify:
        ...
        countfail+=1
    # print(file_name,"cost time:",endsign-startsign+enend-enstart+deend-destart)
    f=open("log-dsa-des.txt",'a',encoding="utf-8")
    f.write("\n\n")
    ...
    f.write("\nsign-time:")
    f.write(str(endsign-startsign))
    f.write("\nencrypt:")
    f.write(str(out_mess))
    f.write("\nencrypt-time:")
    ...
    f.write("\ntime-all:")
    f.write(str(endsign-startsign+enend-enstart+deend-destart))
    f.write("\n")
    f.close()
    print()
f=open("log-dsa-des.txt",'a')
f.write("\nall json accuracy:")
f.write(str(countacc/(countacc+countfail)))
f.write("\nall json cost time:")
f.write(str(timesign+timedecrypt+timeencrypt))
f.close()
print("all json accuracy:",countacc/(countacc+countfail))
print("all json cost time:",timesign+timedecrypt+timeencrypt)
